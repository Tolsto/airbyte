...
| EKS: authenticate as an IAM role through IRSA | `AWS` | The AWS credential chain exchanges a Kubernetes token through STS `AssumeRoleWithWebIdentity`. The driver uses the resulting temporary IAM-role credentials to create a SigV4 attestation. Node metadata is not required for the credential exchange. See [AWS SDK compatibility](#aws-sdk-compatibility). |
...
Direct Kubernetes federation and IRSA can both start with a Kubernetes token, but Snowflake ultimately trusts different identities:

```text
OIDC: Kubernetes JWT -> Snowflake validates cluster issuer and service-account subject
AWS with IRSA: Kubernetes JWT -> AWS STS -> temporary IAM-role credentials
               -> driver creates SigV4 attestation -> Snowflake validates IAM identity
AZURE: managed-identity endpoint -> Entra token -> Snowflake validates Entra identity
```

For direct Kubernetes authentication, use `OIDC` on either AKS or EKS. Choose `AWS` on EKS when access should instead be tied to an IAM role obtained through IRSA, subject to the SDK requirements below.

With IRSA, provide `AWS_ROLE_ARN`, `AWS_WEB_IDENTITY_TOKEN_FILE`, and `AWS_REGION` to the destination connector workload. The driver attempts to discover the region through EC2 metadata when `AWS_REGION` is absent, even when it can obtain credentials through IRSA. The AWS default credential chain also checks environment credentials, Java system properties, and credential profiles. Credentials earlier in the chain can take precedence over the intended workload identity, so do not inject static access keys when the intention is to use the pod's IAM role.
...
Do not put tokens, passwords, private keys, authenticator overrides, or user/account overrides in `jdbc_url_params`. Workload identity authentication rejects these conflicting parameters. Other JDBC options continue to work.
...
The Snowflake driver and the connector's explicit STS dependency use AWS SDK **1.12.655**. That SDK contains the web-identity credential provider, but AWS's [documented Java SDK minimum for IRSA](https://docs.aws.amazon.com/eks/latest/userguide/iam-roles-for-service-accounts-minimum-sdk.html) is **1.12.782**. The bundled version is below that documented minimum. Use `OIDC` for direct EKS service-account authentication without relying on the AWS SDK credential chain. IRSA and EKS Pod Identity are distinct mechanisms; IRSA support does not imply EKS Pod Identity compatibility.
